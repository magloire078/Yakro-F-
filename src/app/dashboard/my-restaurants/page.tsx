        ...
        <div className="min-h-screen bg-white pb-32 overflow-x-hidden">
            ...
            <div className="relative h-[35vh] md:h-[55vh] w-full overflow-hidden">
                <Image
                    ...
                    fill
                    className="object-cover scale-105 animate-slow-zoom opacity-10"
                    priority
                />
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-12 relative z-10">
                {myRestaurants.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {myRestaurants.map((restaurant, index) => {
                            ...
                            return (
                                ...
                                >
                                    <div className="bg-white/70 backdrop-blur-xl border border-slate-200/60 transition-all duration-500 group-hover:border-orange-500/50 overflow-hidden h-full flex flex-col shadow-xl group-hover:shadow-orange-500/10 rounded-2xl">
                                        {/* Image Area */}
                                        <div className="relative h-64 w-full overflow-hidden">
                                            ...
                                            <div className="absolute inset-0 bg-gradient-to-t from-white via-white/5 to-transparent opacity-60" />
                                            
                                            {/* Status Overlay */}
                                            <div className="absolute top-6 left-6">
                                                <div className="px-4 py-1.5 bg-white/80 backdrop-blur-xl border border-slate-200/50 rounded-full flex items-center gap-2 shadow-sm">
                                                    <div className="h-1.5 w-1.5 rounded-full bg-green-500 animate-pulse" />
                                                    <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-900">Opérationnel</span>
                                                </div>
                                            </div>
                                        ...
                                        <div className="p-8 flex-grow flex flex-col">
                                            <div className="mb-6">
                                                <h3 className="text-3xl font-black italic uppercase tracking-tighter text-slate-900 leading-tight group-hover:text-orange-500 transition-colors duration-300">
                                                    {restaurant.nom}
                                                </h3>
                                                <div className="flex items-center gap-2 mt-3 opacity-80 group-hover:opacity-100 transition-opacity">
                                                    <MapPin className="h-3 w-3 text-orange-500" />
                                                    <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-500">{restaurant.adresse || 'Emplacement non défini'}</p>
                                                </div>
                                            </div>

                                            <div className="flex items-center gap-6 mt-2 mb-8 p-4 bg-slate-50/50 border border-slate-100 rounded-xl">
                                                <div className="flex flex-col">
                                                    <span className="text-[9px] font-black uppercase tracking-[0.2em] text-slate-400">Cuisine</span>
                                                    <span className="text-xs font-black text-slate-900 uppercase italic tracking-wider">{restaurant.cuisine}</span>
                                                </div>
                                                <div className="h-8 w-[1px] bg-slate-200" />
                                                <div className="flex flex-col">
                                                    <span className="text-[9px] font-black uppercase tracking-[0.2em] text-slate-400">Note</span>
                                                    <div className="flex items-center gap-1.5">
                                                        <span className="text-sm font-black text-slate-900">{restaurant.note || '0.0'}</span>
                                            ...
                                            <div className="mt-auto flex items-center gap-4 pt-6 border-t border-slate-100">
                                                <Button 
                                                    variant="outline" 
                                                    asChild 
                                                    className="flex-1 rounded-xl border-slate-200 bg-white hover:bg-orange-500 hover:border-orange-500 hover:text-white transition-all duration-300 font-black uppercase tracking-tighter text-[11px] h-14"
                                                >
                                                    ...
                                                    return (
                                                        <AlertDialog>
                                                            <AlertDialogTrigger asChild>
                                                                <Button 
                                                                    variant="ghost" 
                                                                    className="h-14 w-14 rounded-xl border border-slate-100 text-slate-400 hover:text-red-500 hover:bg-red-50 transition-all duration-300"
                                                                    disabled={isDeleting === restaurant.id}
                                                                >
                                                                    <Trash2 className="h-5 w-5" />
                                                                </Button>
                                                            </AlertDialogTrigger>
                                                            <AlertDialogContent className="bg-white/90 border border-slate-200 text-slate-900 rounded-2xl max-w-md backdrop-blur-2xl shadow-2xl">
                                                                <AlertDialogHeader>
                                                                    <AlertDialogTitle className="font-black italic uppercase tracking-tighter text-3xl flex items-center gap-3 text-slate-900">
                                                                        <AlertTriangle className="h-8 w-8 text-orange-500" />
                                                                        {hasPlats ? 'ALERTE CRITIQUE' : 'DISSOLUTION'}
                                                                    </AlertDialogTitle>
                                                                    <AlertDialogDescription className="text-slate-500 font-medium pt-4 text-base leading-relaxed italic">
                                                                        {hasPlats ? (
                                                                            <>
                                                                                Cet établissement contient encore des créations culinaires actives. 
                                                                                <span className="block mt-4 text-slate-900 font-bold uppercase tracking-widest text-xs">Veuillez purger le menu avant de pouvoir dissoudre cet établissement.</span>
                                                                            </>
                                                                        ) : (
                                                                            <>
                                                                                Cette action est irréversible. Vous êtes sur le point de dissoudre 
                                                                                <strong className="text-slate-900 uppercase tracking-tighter"> {restaurant.nom}</strong> et d&apos;effacer son héritage numérique du registre Yakro.
                                                                            </>
                                                                        )}
                                                                    </AlertDialogDescription>
                                                                </AlertDialogHeader>
                                                                <AlertDialogFooter className="mt-10 gap-4">
                                                                    <AlertDialogCancel className="rounded-xl border-slate-200 bg-slate-50 hover:bg-slate-100 text-slate-900 uppercase font-bold tracking-widest text-[10px] h-12">ANNULER</AlertDialogCancel>
                                                                    {hasPlats ? (
                    ...
                    <div className="max-w-2xl mx-auto bg-white/70 backdrop-blur-xl border border-slate-100 p-16 text-center shadow-2xl relative overflow-hidden rounded-3xl mt-12">
                        <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-orange-500 to-transparent" />
                        <UtensilsCrossed className="h-20 w-20 text-orange-500/20 mx-auto mb-8" />
                        <h2 className="text-4xl font-black italic uppercase tracking-tighter text-slate-900 mb-4">Empire Vierge</h2>
                ...
                <div className="hidden md:flex justify-center mt-12">
                    <Button asChild className="h-16 px-12 bg-orange-500 hover:bg-orange-600 text-white rounded-2xl font-black italic uppercase tracking-tighter text-lg shadow-[0_20px_40px_rgba(249,115,22,0.2)] transition-all duration-500 hover:scale-105">
